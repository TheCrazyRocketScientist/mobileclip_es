...
def main():
    ...
    parser.add_argument('--neg_calibration', action='store_true',
                        help='Subtract mean negative CAM from positive CAMs. '
                             'Suppresses systematic false positives that fire '
                             'on both positive and negative images.')
    parser.add_argument('--n_neg', type=int, default=50,
                        help='Number of negative images to average for '
                             'calibration map (default 50)')
    ...
    # -----------------------------------------------------------------------
    # 8. Negative calibration — compute mean CAM over negative images
    # -----------------------------------------------------------------------
    neg_cam_mean = None
    if args.neg_calibration and args.train_csv is not None:
        df_all  = pd.read_csv(args.train_csv, index_col=0)
        neg_df  = df_all[df_all['label'] == 0].sample(
            n=min(args.n_neg, (df_all['label'] == 0).sum()),
            random_state=42
        )
        print(f"\n=== Negative calibration: averaging {len(neg_df)} negative CAMs ===")
        neg_cams = []
        tf_neg   = get_transform(512)
        for _, row in tqdm(neg_df.iterrows(), total=len(neg_df),
            ...
            if not os.path.exists(path):
                continue
            try:
                pil_n = Image.open(path).convert('RGB')
                t_n   = tf_neg(pil_n).unsqueeze(0).to(device)
                attn_captured.clear()

                if args.cam_method == 'gradcam':
                    out = grad_runner(input_tensor=t_n,
                                      targets=[target], target_size=None)
                    cam_n = (out[0] if isinstance(out, tuple) else out).squeeze()
                elif args.cam_method == 'eigencam':
                    with torch.no_grad():
                        out = eigen_runner(input_tensor=t_n,
                                           targets=None, target_size=None)
                    cam_n = (out[0] if isinstance(out, tuple) else out).squeeze()
                else:  # hybrid
                    with torch.no_grad():
                        eout = eigen_runner(input_tensor=t_n,
                                            targets=None, target_size=None)
                    ecam = (eout[0] if isinstance(eout, tuple) else eout).squeeze()
                    attn_captured.clear()
                    gout = grad_runner(input_tensor=t_n,
                                       targets=[target], target_size=None)
                    ...
                    def _n01(x):
                        mn, mx = x.min(), x.max()
                        return (x - mn) / (mx - mn + 1e-8)
                    cam_n = _n01((_n01(ecam)**a) * (_n01(gcam)**(1-a)))

                # Normalise to [0,1] before accumulating
                mn, mx = cam_n.min(), cam_n.max()
                cam_n_norm = (cam_n - mn) / (mx - mn + 1e-8)
                if isinstance(cam_n_norm, torch.Tensor):
                    neg_cams.append(cam_n_norm.numpy())
                else:
                    neg_cams.append(cam_n_norm)
            except Exception as e:
                pass

        if neg_cams:
            neg_cam_mean = np.mean(neg_cams, axis=0)  # (16, 16)
            neg_cam_mean = neg_cam_mean.astype(np.float32)
            print(f"  Calibration map computed from {len(neg_cams)} negatives")
            print(f"  Calibration map range: [{neg_cam_mean.min():.4f}, "
                  f"{neg_cam_mean.max():.4f}]")
        else:
            print("  [WARN] No negative CAMs computed — skipping calibration")
    ...
    for rec in tqdm(image_list, desc='Generating CAMs'):
        ...
        try:
            ...
            # Apply negative calibration if available
            if neg_cam_mean is not None:
                # Resize calibration map to match current CAM size
                cal = cv2.resize(neg_cam_mean,
                                 (grayscale_cam.shape[1], grayscale_cam.shape[0]))
                grayscale_cam = grayscale_cam - cal
                # Clip and renormalise to [0,1]
                grayscale_cam = np.clip(grayscale_cam, 0, None)
                mn, mx = grayscale_cam.min(), grayscale_cam.max()
                if mx > mn:
                    grayscale_cam = (grayscale_cam - mn) / (mx - mn)
