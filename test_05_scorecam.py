...
test_05_scorecam.py
-------------------
Score-CAM on 5 positive SIIM images.
Score-CAM: masks each feature map channel, measures score drop.
No gradients needed — more faithful than GradCAM but slow (256 passes/image).

Usage:
    python test_05_scorecam.py \
...
def score_cam(image_encoder, img_tensor, target_class, target_layer, device):
    """
    Score-CAM implementation.
    For each channel in the target layer activation:
      1. Upsample channel mask to input size
      2. Multiply with input image (mask out everything else)
      3. Forward pass → get class score
    Weight each channel by its score, sum.
    """
    activations = {}
    def hook(m, i, o): activations["feat"] = o.detach()
    h = target_layer.register_forward_hook(hook)

    with torch.no_grad():
        _ = image_encoder(img_tensor)
    h.remove()
    ...
    scores = []
    for c in tqdm(range(C), desc="Score-CAM channels", leave=False):
        channel = feat[0, c]  # (H, W)
        # Normalise channel to [0,1]
        mn, mx = channel.min(), channel.max()
        if mx - mn < 1e-8:
            scores.append(0.0)
            continue
        mask = (channel - mn) / (mx - mn)
        ...
        # Mask input
        masked_input = img_tensor * mask_up
        with torch.no_grad():
            out = image_encoder(masked_input)
        score = out[0, target_class].item()
        scores.append(score)

    scores_t = torch.tensor(scores).to(device)  # (C,)
    # Weighted sum of channels
    scores_t = F.relu(scores_t)
    weights   = scores_t / (scores_t.sum() + 1e-8)
    cam = (weights.view(C,1,1) * feat[0]).sum(0)  # (H,W)
...
def main():
    ...
    parser.add_argument("--out_dir",      default="./test_05_scorecam")
    ...
    print(f"Classes: 1 fg + {len(bg_features)} bg = {len(all_features)} total")
    ...
    for _, row in df.iterrows():
        ...
        print(f"\nScore-CAM: {stem[-25:]}")
        cam = score_cam(image_encoder, tensor, target_class=0, target_layer=target_layer, device=device)
        ...
        save_panel(path, cam_full, gt_mask, os.path.join(args.out_dir, f"{stem}_scorecam.jpg"))
